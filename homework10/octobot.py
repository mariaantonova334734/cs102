...
def choose_deadline_action(message):
    ...
    if message.text == "Удалить дедлайн":
        info = bot.send_message(
            message.chat.id, "Введите номер дедлайна, который  вы хотите удалить", reply_markup=None
        )
...
def choose_removal_option(message):
    ...
    info = bot.send_message(
        message.chat.id, "Введите да, если вы точно хотите удалить все", reply_markup=None
    )
...
def add_new_subject_url(message):
    ...
    if not validators.url(new_subject_url):
        info = bot.send_message(
            message.chat.id, "Неверная ссылка. Введите ссылку еще раз", reply_markup=None
        )
        bot.register_next_step_handler(info, add_new_subject_url)
        return
...
def add_new_deadline(message):
    ...
    if date == False:
        info = bot.send_message(
            message.chat.id, "Введен неправильный формат даты. Введите заново", reply_markup=None
        )
        bot.register_next_step_handler(info, add_new_deadline)
        return
...
def update_subject(message):
    ...
    if subject_is_found:
        ...
        info = bot.send_message(
            message.chat.id, "Вы ввели название несуществующего предмета\nВыберите предмет из меню"
        )
        bot.register_next_step_handler(info, update_subject)
        return
