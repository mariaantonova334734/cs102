...
subject_dict = {}  #словарь для хранения полльзовательской информации из разных чатов, ключ = chat.id, а значение - выбранный предмет

deadline_dict = {} #словарь для хранения полльзовательской информации из разных чатов, ключ = chat.id, а значение - номер дедлайна
...
def connect_table(message):
    ...
    if mo is None:  #если неправильна введена ссылка на таблицу
        info = bot.send_message(message.chat.id,"ID таблицы не обнаружен, введите ссылку корректно")
...
def choose_action(message):
    ...
    if message.text == "Подключить Google-таблицу":
        info = bot.send_message(message.chat.id, "Введите ссылку на таблицу", reply_markup=None)
        bot.register_next_step_handler(info, connect_table)
        return
        ...
        info = bot.send_message(message.chat.id, "Вы хотите внести новый предмет или редактировать текущий?", reply_markup=start_markup)
        ...
        info = bot.send_message(message.chat.id, "Выберите предмет для редактирования дедлайна",
        ...
        deadl = ""
        for strok in range(2, len(worksheet.col_values(1)) + 1): #проходимся по номерам строк предметов
            for current_d in worksheet.row_values(strok)[2:]: #проходимся по ряду
                ...
                if date_of_deadline != False:
                    if nedel >= date_of_deadline >= date_today:
                        deadl += worksheet.cell(strok, 1).value + " : " + current_d +"\n" #предмет +дата дедалайна
...
def choose_subject_action(message):
    ...
    if message.text == "Внести новый предмет":
        ...
        info = bot.send_message(message.chat.id, "Выберите редактируемый предмет",
                                reply_markup=start_markup)
        ...
        info = bot.send_message(message.chat.id, "Выберите название предмета, который вы хотите удалить",
                                reply_markup=start_markup)
...
def choose_deadline_action(message):
    ...
    if message.text == "Удалить дедлайн":
        info = bot.send_message(message.chat.id, "Введите номер дедлайна, который  вы хотите удалить", reply_markup=None)
        bot.register_next_step_handler(info, delete_deadline)
        return
    elif message.text == "Обновить дедлайн":
        info = bot.send_message(message.chat.id, "Введите номер дедлайна, который хотите отредактировать", reply_markup=None)
        bot.register_next_step_handler(info, update_subject_deadline)
        return
    elif message.text == "Добавить дедлайн":
        info = bot.send_message(message.chat.id, "Введите дату нового дедлайна в формате 01/01/00 (ДД/ММ/ГГ)", reply_markup=None)
...
def choose_subject(message):
    ...
    info = bot.send_message(message.chat.id, "Выберите действие для дедлайна",
                            reply_markup=start_markup)
    bot.register_next_step_handler(info, choose_deadline_action)
    return
...
def update_subject_deadline(message):
    ...
    try:
        ...
        chislo = int(number)
        if chislo < 1:
            info = bot.send_message(message.chat.id, "Число должно быть больше нуля. Введите число еще раз", reply_markup=None)
            bot.register_next_step_handler(info, update_subject_deadline)
            return

    except:
        info = bot.send_message(message.chat.id, "Это не целое число. Введите целое число", reply_markup=None)
        bot.register_next_step_handler(info, update_subject_deadline)
        return

    deadline_dict[message.chat.id] = number

    info = bot.send_message(message.chat.id, "Введите новую дату дедлайна в формате 01/01/00 (ДД/ММ/ГГ)", reply_markup=None)
    bot.register_next_step_handler(info, add_new_deadline)
    return
...
def add_new_subject(message):
    ...
    if subject_is_found: #если нашли такой же предмет
        info = bot.send_message(message.chat.id,
                                "Вы ввели название существующего предмета. Введите название нового предмета", reply_markup=None)
        bot.register_next_step_handler(info, add_new_subject)
        return
    else:
...
def add_new_subject_url(message):
    ...
    if not validators.url(new_subject_url):
        info = bot.send_message(message.chat.id,
                                "Неверная ссылка. Введите ссылку еще раз", reply_markup=None)
        bot.register_next_step_handler(info, add_new_subject_url)
        return
...
def add_new_deadline(message):
    ...
    if date == False:
        info = bot.send_message(message.chat.id,
                                "Введен неправильный формат даты. Введите заново", reply_markup=None)
        bot.register_next_step_handler(info, add_new_deadline)
        return

    current_date = datetime.today()
    if date<current_date:
        info = bot.send_message(message.chat.id, "Дата дедлайна уже прошла, введите новую дату", reply_markup=None)
        bot.register_next_step_handler(info, add_new_deadline)
        return
    if date>current_date.replace(year = current_date.year + 1):
        info = bot.send_message(message.chat.id, "Дата дедлайна слишком далеко, учебный год уже закончится. Введите новую дату", reply_markup=None)
        bot.register_next_step_handler(info, add_new_deadline)
        return
...
def edit_subject_name(message):
    ...
    subject_is_found, cell_row = find_subject_row(current_subj)
    if subject_is_found:
        worksheet.update_cell(cell_row, 1, new_subj)  # добавляем (заменяем название) предмета в таблице
...
def delete_deadline(message):
    ...
    try:
        ...
        if chislo < 1:
            info = bot.send_message(message.chat.id, "Число должно быть больше нуля. Введите число еще раз",
                                    reply_markup=None)
            bot.register_next_step_handler(info, update_subject_deadline)
            return
...
def vedomosti_of_subjects(message):
    ...
    worksheet, url, df = access_current_sheet()
    for i in df.index:
        predm = telebot.types.InlineKeyboardButton(df.loc[i, 'subject'], url=df.loc[i, 'link'])
        markup.add(predm)
...
def start(message):
    ...
    bot.register_next_step_handler(info, choose_action) #ждёт сообщение пользователя и потом вызывает указанную функцию с аргументом message
