...
def update_news():
    ...
    for novost in news_list:
        news = News(title=novost[0],
                    author=novost[1],
                    url=novost[2],
                    comments=novost[3],
                    points=novost[4])
        #проверяем, нет ли такой новости в базе данных
        samenews = s.query(News).filter(News.title == news.title).filter(News.author == news.author).all()
        if samenews == []:
            s.add(news)
    s.commit()
...
def classify_news():
    ...
    set_of_news = s.query(News).filter(News.label != None).all() #сет новостей, ни которые была реакция
    model.fit(
        [clean(news.title).lower() for news in set_of_news],
        [news.label for news in set_of_news],
    )   #моделирование прогнозов
    set_no_reaction = s.query(News).filter(News.label == None).all() #сет новостей, ни которые не было реакции
    #сортировка в зависимости от предсказанной реакции
    return template(
        "news_template",
        rows=sorted(
            set_no_reaction, key=lambda news: type_of_label(
                model.predict(clean(news.title).lower())) #получение прогнозов
